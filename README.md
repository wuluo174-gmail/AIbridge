...
每个浏览器 Tab 是独立的协商会话，可同时处理多个不同任务。会话状态（事件流、进程、历史记录）完全隔离，互不干扰。URL 自动携带 `?sid=xxx` 参数，刷新页面可恢复会话。
...
Claude Code 在 plan 模式下生成的计划文档通过快照差集方案关联：启动前快照 `~/.claude/plans/` 文件集合，结束后取差集找到新文件。多个协商并发时不会抓错。
