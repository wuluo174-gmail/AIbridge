...
def run_execution(sess, *,
    ...
    """
    try:
        add_event(sess, "status_change", {"status": "executing", "msg": "正在执行..."})
...
def run_first_review(sess, approved_plan, *,
    ...
    try:
        ...
        sess.review_round = 1
        add_event(sess, "status_change", {"status": "review_pending", "msg": "正在评审执行结果..."})
        ...
        if exec_reviewer_adapter.detect_closure(review):
            with sess.status_lock:
                sess.status = "done"
            add_event(sess, "review_done", {"round": 1, "msg": "执行审查确认任务收口成功。", "success": True})
        else:
            with sess.status_lock:
                sess.status = "review_fix"
            add_event(sess, "review_needs_fix", {"round": 1, "msg": "执行审查发现问题，等待你确认是否修复。", "review": review})
...
def run_review_fix_cycle(sess, *,
    ...
    try:
        ...
        if exec_reviewer_adapter.detect_closure(review):
            with sess.status_lock:
                sess.status = "done"
            add_event(sess, "review_done", {"round": rr, "msg": f"第 {rr} 轮确认任务收口成功。", "success": True})
        else:
            with sess.status_lock:
                sess.status = "review_fix"
            add_event(sess, "review_needs_fix", {"round": rr, "msg": "仍发现问题，等待你确认是否继续修复。", "review": review})
