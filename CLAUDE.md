...
针对本项目，以上三问必须落到下面这些一等对象上，而不是落到 UI 补丁上：
...
1. `Process` 只读事件，不准从结果反推过程。
2. `Result` 只读 artifact，不准从日志拼最终输出。
3. 用户输入必须是独立 intervention，不准混进 history 末尾连续 user 条目。
4. 角色必须显式建模为 `planner/reviewer/executor/validator`，不准再从二角色模型临时推导。
5. `terminal` 和 `scene` 只是同一账本的两种投影，不准各自维护独立真相源。
